...
void _help()
{
    ...
    printf("where           同步数据的条件，即select语句的where部分。本参数可以为空，如果为空，表示同步全部的记录。\n");
...
bool _syncupdate()
{
    ...
    CREATE TABLE `LK_ZHOBTCODE1` (
      `obtid` varchar(10) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '站点代码',
      `cityname` varchar(30) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '城市名称',
      `provname` varchar(30) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '省名称',
      `lat` int(11) NOT NULL COMMENT '纬度，单位：0.01度。',
      `lon` int(11) NOT NULL COMMENT '经度，单位：0.01度。',
      `height` int(11) NOT NULL COMMENT '海拔高度，单位：0.1米。',
      `upttime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间。',
      `keyid` int(11) NOT NULL AUTO_INCREMENT COMMENT '记录编号，自动增长列。',
      PRIMARY KEY (`obtid`),
      UNIQUE KEY `ZHOBTCODE1_KEYID` (`keyid`)
    ) ENGINE=FEDERATED CONNECTION='mysql://root:DYT.9525ing@127.0.0.1:3306/TestDB/T_ZHOBTCODE1';
    */
    return true;
}
