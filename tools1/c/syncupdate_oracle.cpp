 ...
 * 3. keyid字段处理
 * 4. 时间格式和时间函数
 * 5. oracle不需要单独为查询语句创建数据库连接
 * 6. oracle不需要MAXPARAMS宏
 * 7. 如果SQL语句有错误，oracle不会在prepare时报错，而是在execute时报错，为了兼容性考虑，尽量不在prepare时判断SQL语句的错误代码
 * 8. oracle与mysql的数据类型关键字不同
 */
...
struct st_arg
{
	...
	char lnktname[31];          // 远程表名，remotetablename + @dblink
...
void _help()
{
	printf("Use:syncupdate_oracle logfilename xmlbuffer\n\n");
	printf("Sample:\n");
	...
	printf("/project/tools1/bin/procctl 10 /project/tools1/bin/syncupdate_oracle /log/idc/syncupdate_oracle_ZHOBTCODE3.log \"<localconnstr>scott/dyting9525@snorcl11g_43</localconnstr><charset>Simplified Chinese_China.AL32UTF8</charset><lnktname>T_ZHOBTCODE1@dbsc_qx</lnktname><localtname>T_ZHOBTCODE3</localtname><remotecols>obtid,cityname,provname,lat,lon,height,upttime,keyid</remotecols><localcols>obtid,cityname,provname,lat,lon,height,upttime,keyid</localcols><where>where obtid like '54%%%%'</where><synctype>2</synctype><remoteconnstr>qxidc/dyting9525@snorcl11g_43</remoteconnstr><remotetname>T_ZHOBTCODE1</remotetname><remotekeycol>obtid</remotekeycol><localkeycol>obtid</localkeycol><maxcount>10</maxcount><timeout>50</timeout><pname>syncupdate_oracle_ZHOBTCODE3</pname>\"\n\n");

	printf("/project/tools1/bin/procctl 10 /project/tools1/bin/syncupdate_oracle /log/idc/syncupdate_oracle_ZHOBTMIND2.log \"<localconnstr>scott/dyting9525@snorcl11g_43</localconnstr><charset>Simplified Chinese_China.AL32UTF8</charset><lnktname>T_ZHOBTMIND1@dbsc_qx</lnktname><localtname>T_ZHOBTMIND2</localtname><remotecols>obtid,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid</remotecols><localcols>obtid,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid</localcols><where>where ddatetime>sysdate-0.04</where><synctype>2</synctype><remoteconnstr>qxidc/dyting9525@snorcl11g_43</remoteconnstr><remotetname>T_ZHOBTMIND1</remotetname><remotekeycol>keyid</remotekeycol><localkeycol>keyid</localkeycol><maxcount>300</maxcount><timeout>50</timeout><pname>syncupdate_oracle_ZHOBTMIND2</pname>\"\n\n");
	...
	printf("lnktname        远程表名，remotetablename + @dblink。\n");
	...
	printf("where           同步数据的条件，本参数可以为空，如果为空，表示同步全部的记录。填充在delete本地表和select 远程表之后。\n"
		   "		注意：1）where中的字段必须同时在本地表和远程表中；2）不要用系统时间作为条件\n");
...
bool _syncupdate_oracle()
{
	...
	if (starg.synctype == 1)
	{
		...
		// 再把lnktname表中满足where条件的记录插入到localtname表中
	...
	// insert into T_ZHOBTCODE3(starg.localcols) select starg.remotecols from starg.lnktname where starg.remotekeycol in strbind
	stmtins.prepare("insert into %s(%s) select %s from %s where %s in (%s)", starg.localtname, starg.localcols, starg.remotecols, starg.lnktname, starg.remotekeycol, strbind);
	for (int ii = 0; ii < starg.maxcount; ii++)
		stmtins.bindin(ii+1, lockeyvalues[ii], 50);
	// logfile.WriteEx("stmtins = insert into %s(%s) select %s from %s where %s in (%s)", starg.localtname, starg.localcols, starg.remotecols, starg.lnktname, starg.remotekeycol, strbind);
	...
	while (true)
	{
		...
		if (count == starg.maxcount)
		{
			...
			// logfile.Write("sync %s to %s (%d rows) in %.2fsec\n", starg.lnktname, starg.localtname, count, Timer.Elapsed());
			connloc.commit();
