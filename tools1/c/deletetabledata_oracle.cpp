 ...
 * · 3. keyid字段处理
 * · 4. 时间函数和时间格式
 * 5. oracle 数据库不需要为了查询语句创建专用的数据库连接
 * 6. oracle 不需要 MAXPARAMS宏
 */
...
int main(int argc, char *argv[])
{
    ...
    if (conn.connecttodb(starg.connectstr, NULL, 1) != 0)
    { logfile.Write("执行select语句的数据库连接失败(%s)\n%s\n", starg.connectstr, conn.m_cda.message); _exit(-1); }

    if (_deletetabledata_oracle() == false)
        _exit(-1);
...
void _help()
{
    printf("\n");
    printf("Use:\n");
    printf("deletetabledata_oracle logfile xmlbuffer");
    printf("Example:\n");
    printf("/project/tools1/bin/procctl 3600 /project/tools1/bin/deletetabledata_oracle /log/idc/deletetabledata_oracle_T_ZHOBTMIND1.log \"<connectstr>qxidc/dyting9525@snorcl11g_43</connectstr><tname>T_ZHOBTMIND1</tname><keycol>keyid</keycol><where>where ddatetime<sysdate-0.03</where><starttime>01,02,03,04,05,13</starttime><timeout>120</timeout><pname>deletetabledata_oracle_ZHOBTMIND1</pname>\"\n\n");
...
bool _isstarttime()
{
    if (strlen(starg.starttime) != 0)
    {
        ...
        LocalTime(time, "hh24");
        if (strstr(starg.starttime, time) == 0)
            return false;
    }
