...
int main(int argc, char *argv[])
{
    ...
    if (conn2.connecttodb(starg.connectstr, NULL) != 0)
    { logfile.Write("执行select语句的数据库连接失败(%s)\n%s\n", starg.connectstr, conn2.m_cda.message); _exit(-1); }

    if (_migratetabledata() == false) _exit(-1);
...
void _help()
{
    printf("\n");
    printf("Use:\n");
    printf("migratetabledata logfile xmlbuffer");
    printf("Example:\n");
    printf("/project/tools1/bin/procctl 3600 /project/tools1/bin/migratetabledata /log/idc/migratetabledata_T_ZHOBTMIND.log \"<connectstr>127.0.0.1,root,DYT.9525ing,TestDB,3306</connectstr><srctname>T_ZHOBTMIND</srctname><dsttname>T_ZHOBTMIND_HIS</dsttname><keycol>keyid</keycol><where>where ddatetime<timestampadd(minute,-120,now())</where><starttime>01,02,03,04,05,13</starttime><maxcount>300</maxcount><timeout>120</timeout><pname>migratetabledata_ZHOBTMIND1</pname>\"\n\n");
...
bool _migratetabledata()
{
    ...
    if (TABCOLS.allcols(&conn1, starg.dsttname) == false)
    {
