...
struct st_arg
{
    ...
    bool checktime;         // 是否需要检查服务端文件的时间，缺省为false
...
void _help()
{
    ...
    printf("    <checktime></checktime>         是否需要检查服务端文件的时间，缺省为false\n");
    printf("    <timeout></timeout>             进程心跳超时时间\n");
    printf("    <pname></pname>                 进程名，建议用\"ftpgetfiles_后缀\"的方式\n\n\n");
}
...
bool _xmltoarg(char *strxmlbuffer)
{
    ...
    GetXMLBuffer(strxmlbuffer, "checktime", &starg.checktime);                  // 是否需要检查服务端文件的时间，缺省为false
