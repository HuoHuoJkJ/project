...
void RecvFileMain()
{
    while (true)
    {
        ...
        if (strcmp(strrecvbuffer, "<activetest>ok</activetest>") == 0)
        {
            ...
            logfile.Write("RecvFileMain---(发送):strsendbuffer=%s\n", strsendbuffer);
            if (TcpServer.Write(strsendbuffer) == false)
        ...
        if (strncmp(strrecvbuffer, "<filename>", 10) == 0)
        {
            ...
            // 接收上传的文件的内容 
            SNPRINTF(strsendbuffer, sizeof(strsendbuffer), 1024, "<filename>%s</filename><result>success</result>", clientfilename);
