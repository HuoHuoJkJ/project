/*
 * 本程序是数据中心的公共功能模块，采用增量的方法同步Oracle数据库之间的表。
 */
...
// 业务处理主函数
bool _syncincrement_oracle(bool &bcontinue);
...
int main(int argc, char *argv[])
{
	...
	{
		// 业务处理主函数
		if (_syncincrement_oracle(bcontinue) == false) _exit(0);
...
void _help()
{
	...
	printf("maxcount        每次执行一次同步操作的记录数，不能超过MAXPARAMS宏（在_mysql.h中定义，默认值为256）。\n");
...
bool _xmltoarg(char *strxmlbuffer)
{
	...
	// 每次执行一次同步操作的记录数，不能超过MAXPARAMS宏（在_mysql.h中定义，默认值为256）。
...
bool _syncincrement_oracle(bool &bcontinue)
{
	...
	// 从远程表查找大于maxkeyvalue的值，增量同步到本地表中
	char remkeyvalue[51];
	...
	// insert into T_ZHOBTCODE3(starg.localcols)
	// 		select starg.remotecols from starg.lnktname
	// 			where starg.remotekeycol in strbind
