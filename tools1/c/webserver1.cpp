...
pthread_spinlock_t vthidlock;   // 自旋锁
...
void *thmain(void *arg)
{
    ...
    // 判断URL中用户名和密码，如果不正确，返回认证失败的响应报文，线程退出
    if (Login(&conn, strgetbuffer, connfd) == false) pthread_exit(0);

    // 判断用户是否有调用接口的权限，如果没有，返回没有权限的响应报文，线程退出
    if (CheckPerm(&conn, strgetbuffer, connfd) == false) pthread_exit(0);
    ...
    // 在执行接口的sql语句，把数据返回给客户端
    if (ExecSQL(&conn, strgetbuffer, connfd) == false) pthread_exit(0);
