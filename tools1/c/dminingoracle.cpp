...
struct st_arg
{
    ...
    char pname[51];         // 进程名，建议采用"dminingoracle_后缀"的方式
} starg;

#define MAXPARAMS   256     // 结果集字段的最大数
// #define MAXFIELDLEN     500     // 结果集字段值的最大长度
int     imaxfieldlen = -1;       // 结果集字段值的最大长度，可以在_XmlToArg函数中动态的调节大小

char strfieldname[MAXPARAMS][31];   // 结果集字段名数据，从starg.fieldstr解析得到
int  ifieldlen[MAXPARAMS];          // 结果集字段的长度数组，从starg.fieldlen解析得到
...
int main(int argc, char *argv[])
{
    ...
    if ( _dminingoracle() == false ) return -1;
...
void _help()
{
    ...
    printf("\n");
    printf("Use: dminingoracle logfilename xmlbuffer\n\n");

    printf("/project/tools1/bin/procctl 3600 /project/tools1/bin/dminingoracle /log/idc/dminingoracle_ZHOBTCODE1.log \"<connstr>qxidc/dyting9525@snorcl11g_43</connstr><charaset>Simplified Chinese_China.AL32UTF8</charaset><selectsql>select obtid,cityname,provname,lat,lon,height from T_ZHOBTCODE1</selectsql><fieldstr>obtid,cityname,provname,lat,lon,height</fieldstr><fieldlen>10,30,30,10,10,10</fieldlen><bfilename>ZHOBTCODE1</bfilename><efilename>HYCZ</efilename><outpath>/idcdata/dmindata1</outpath><timeout>30</timeout><pname>dminingoracle_ZHOBTCODE1</pname>\"\n\n");
    printf("/project/tools1/bin/procctl 30 /project/tools1/bin/dminingoracle /log/idc/dminingoracle_ZHOBTMIND1.log \"<connstr>qxidc/dyting9525@snorcl11g_43</connstr><charaset>Simplified Chinese_China.AL32UTF8</charaset><selectsql>select obtid,to_char(ddatetime,'yyyymmddhh24miss'),t,p,u,wd,wf,r,vis,keyid from T_ZHOBTMIND1 where keyid>:1 and ddatetime>sysdate-0.04</selectsql><fieldstr>obtid,ddatetime,t,p,u,wd,wf,r,vis,keyid</fieldstr><fieldlen>10,19,8,8,8,8,8,8,8,15</fieldlen><bfilename>ZHOBTMIND1</bfilename><efilename>HYCZ</efilename><outpath>/idcdata/dmindata1</outpath><starttime></starttime><maxcount>1000</maxcount><incfield>keyid</incfield><incfieldname>/idcdata/dmining/dminingoracle_ZHOBTMIND1_HYCZ.list</incfieldname><connstr1>qxidc/dyting9525@snorcl11g_43</connstr1><timeout>30</timeout><pname>dminingoracle_ZHOBTMIND1_HYCZ</pname>\"\n\n");

    printf("本程序是通用的功能模块，用于从oracle数据源表抽取数据，生成xml文件。\n");
    ...
    printf("  pname           进程名，建议采用\"dminingoracle_后缀\"的方式\n\n\n");
}
...
bool _XmlToArg(char *strxmlbuffer)
{
    ...
    if ((ifieldcount = CmdStr.CmdCount()) > MAXPARAMS)
    { logfile.Write("fieldlen的字段数太多，超出了最大限制%d\n", MAXPARAMS); return false; }

    for (int ii = 0; ii < ifieldcount; ii++)
    {
        CmdStr.GetValue(ii, &ifieldlen[ii]);
        // if (ifieldlen[ii] > imaxfieldlen) ifieldlen[ii] = imaxfieldlen;
        if (ifieldlen[ii] > imaxfieldlen) imaxfieldlen = ifieldlen[ii];
    }
...
bool _dminingoracle()
{
    ...
    // 绑定输出变量
    char strfieldvalue[ifieldcount][imaxfieldlen+1];
